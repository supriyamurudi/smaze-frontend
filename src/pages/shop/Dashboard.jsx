...
// ==========================================
// SIDEBAR COMPONENT (Placeholder - connects to your layout)
// ==========================================
const Sidebar = ({ shopName }) => {
  ...
  return (
    <motion.aside
      ...
      className="fixed top-0 left-0 z-40 hidden h-full w-64 flex-col border-r border-slate-200 bg-white shadow-xl lg:flex"
    >
      {/* Sidebar Header */}
      <div className="flex items-center gap-3 border-b border-slate-100 p-6">
        <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-violet-600 to-purple-600">
          <HiOutlineBuildingStorefront className="text-white" size={24} />
        </div>
        <div className="min-w-0">
          <h2 className="truncate text-lg font-black text-slate-800">
            {shopName || "Shop"}
          </h2>
          <p className="text-xs text-slate-500">Merchant Panel</p>
        </div>
      </div>
...
// ==========================================
// SKELETON LOADER (For the main content area)
// ==========================================
  ...
  >
    <div className="w-full max-w-md rounded-2xl border border-amber-200 bg-amber-50 p-5 text-center sm:p-6">
      ...
      <p className="mb-4 text-sm text-amber-700 sm:text-base">
        {message || "Your shop is currently under review by our admin team."}
      </p>
    ...
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex flex-wrap items-center gap-2 sm:flex-shrink-0">
        ...
        <span className="inline-flex items-center rounded-full bg-emerald-400/30 px-3 py-1.5 text-xs font-semibold backdrop-blur-sm">
          <span className="mr-2 h-2 w-2 rounded-full bg-emerald-400" /> Active
        </span>
      </div>
    </div>
  </motion.div>
    ...
    {growth && (
      <div className="relative mt-2">
        <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-emerald-600">
          <HiOutlineArrowTrendingUp size={12} /> {growth}
        </span>
      </div>
    )}
  </motion.div>
...
export default function Dashboard() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen overflow-x-hidden bg-gradient-to-br from-slate-50 via-white to-violet-50/30 pb-16 sm:pb-20">
        <Sidebar shopName="Loading..." />
        <div className="ml-0 w-full lg:ml-64">
  ...
  if (pending) return <PendingApproval message={error} />;
  if (error)
    return (
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-gradient-to-br from-slate-50 via-white to-violet-50/30 pb-16 sm:pb-20">
      <Sidebar shopName={dashboardData?.shop?.name || "Shop"} />

      {/* Main content pushes right to accommodate the sidebar */}
      <div className="ml-0 w-full lg:ml-64">
        <div className="mx-auto w-full max-w-7xl px-3 py-4 sm:px-6 sm:py-8 lg:px-8">
                ...
                recentOffers.map((offer) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span
                        className={`inline-flex flex-shrink-0 items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-semibold sm:px-3 sm:text-xs ${isActive ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700"}`}
                      >
                        <span
